...
const INSTRUCCIONES = `Eres un asesor inmobiliario experto en marketing digital en Centroamérica (tono
...
- Termina con una llamada a la acción breve invitando a escribir o agendar visita.
...
- No inventes datos que no estén en la ficha (no inventes dirección exacta si no se dio, no
  inventes amenidades que no aparecen).
  ...
  try {
    ...
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 700,
        temperature: 0.8,
        messages: [{ role: "user", content: INSTRUCCIONES + resumen }],
      }),
    });
